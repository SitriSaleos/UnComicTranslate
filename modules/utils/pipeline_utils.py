...
def has_access(credentials, service, key_field):
...
def has_all_credentials(credentials, service, keys):
...
def validate_ocr(main_page, source_lang):
    ...
    if ocr_tool == tr("Microsoft OCR"):
        ...
        if not has_all_credentials(credentials, service, ['api_key_ocr', 'endpoint']):
    ...
    # Google Cloud Vision
        ...
        if not has_access(credentials, service, 'api_key'):
            Messages.show_signup_or_credentials_error(main_page)
            return False
...
def validate_translator(main_page, source_lang, target_lang):
    ...
    if translator_tool == tr("DeepL"):
        ...
        service = tr("Microsoft Azure")
        if not has_access(credentials, service, 'api_key_translator'):
            Messages.show_signup_or_credentials_error(main_page)
            return False
    ...
    elif "Gemini" in translator_tool or "Gemma" in translator_tool:
        service = tr('Google Gemini')
        if not has_access(credentials, service, 'api_key'):
            Messages.show_signup_or_credentials_error(main_page)
            return False
        
    elif translator_tool == tr("DeeLX"):
        service = tr("DeeLX")
        if not has_access(credentials, service, 'url'):
            Messages.show_signup_or_credentials_error(main_page)
            return False
